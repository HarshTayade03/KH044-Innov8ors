"""Acceptance tests for safe offline validation, evidence artifacts, and LLM prioritization."""
import hashlib
import pytest
...
def test_three_scenarios_round_trip_and_hash_integrity(finding_factory, cwe, scenario):
    ...
    artifacts = validation_repo.list_artifacts(result.validation_id)
    assert stored == result
    assert len(artifacts) == 4
    artifact_types = {a.artifact_type for a in artifacts}
    assert artifact_types == {"http_request", "http_response", "execution_log", "validation_summary"}
    for artifact in artifacts:
        assert artifact.content_hash == hashlib.sha256(artifact.content.encode("utf-8")).hexdigest()
        assert artifact.content_size == len(artifact.content.encode("utf-8"))
...
def test_evidence_redaction_api_and_risk_integration(finding_factory):
    ...
    for art in evidence.json()["artifacts"]:
        content = art["content"]
    ...
    assert "llm_context" in rescored.factors
    assert rescored.factors["llm_context"]["evidence_basis"]
    assert rescored.factors["llm_context"]["uncertainty"]
...
def test_batch_validation_and_history_endpoints(finding_factory):
    ...
    assert batch_results["failures"] == []

    list_resp = client.get(f"/api/v1/canonical-issues/{issue1.canonical_issue_id}/validations")
    assert list_resp.status_code == 200
    assert len(list_resp.json()) >= 1

    all_resp = client.get("/api/v1/validations")
...
def test_batch_validation_reports_rejections_without_aborting_successes(finding_factory):
    ...
    assert response.status_code == 200
    body = response.json()
    assert any(item["canonical_issue_id"] == allowed.canonical_issue_id for item in body["results"])
    assert any("allowlist" in item["error"] for item in body["failures"])
