...
class ValidationRepository:
    ...
    def list_artifacts(self, validation_id: str) -> list[Artifact]:
        ...
        artifacts = [
            Artifact(artifact_id=r["artifact_id"], validation_id=validation_id, artifact_type=r["artifact_type"], content=r["content"], content_hash=r["content_hash"], content_size=r["content_size"], redacted=bool(r["redacted"]), metadata=json.loads(r["metadata"]), created_at=datetime.fromisoformat(r["created_at"]))
            for r in rows
        ]
        for artifact in artifacts:
            retained = artifact.content.encode("utf-8")
