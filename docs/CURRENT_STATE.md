...
| M4 / P5 prioritization | `schemas/risk.py`, `services/risk_engine.py`, `services/threat_intel.py`, `repositories/risk_repo.py`, priority routes in `api/cases.py`, `api/threat_intel.py` | CISA KEV, NIST NVD, and FIRST EPSS feed contracts are exposed with source/mode status. Local mock KEV/EPSS remains the deterministic default; explicit live per-CVE NVD/EPSS refresh is available only when enabled. Weighted score, three tiers, explanations, and source fingerprints are retained. |
...
| M6 / P7 cases | DB tables `cases`, `reviews`, `audit_events`; `schemas/case.py`, `repositories/case_repo.py`, `services/case_service.py`, case routes | Case assembly, active/retired and stale guards, transactional human review, review history, resolved-state retirement, and append-only audit inspection are implemented. Resolved canonical issues leave active queues while evidence and case history remain retrievable. |
| F0 / analyst console | `static/index.html`, `dashboard.css`, `dashboard.js` | Implemented and syntax-checked console for ingestion, views, embeddings, deduplication, risk, searchable case queue/detail, approve/request-evidence/resolve actions, offline validation/evidence display, filterable audit log, and JSON worklist export. |
| M7 / P8 dashboard | `api/dashboard.py`, `repositories/dashboard_repo.py`, and F0 console | Partial: truthful aggregate metrics, provenance labels, case workflow, global audit visibility, and analyst worklist export are implemented; authentication, server-side analyst identity, richer executive analytics, and broad pagination remain pending. |
...
- Audit: `GET /audit-events` with entity and actor filters plus pagination.
- Threat intelligence: `GET /threat-intelligence/feeds`; explicit `POST /threat-intelligence/refresh?cve_id=...` when live refresh is enabled.
